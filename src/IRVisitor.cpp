...
void IRVisitor::visit(const DoWhile *op) {
  op->min.accept(this);
  op->cond.accept(this);
  op->step.accept(this);
  op->body.accept(this);
...
void IRVisitor::visit(const TensorOp *op) {
  for (size_t i = 0; i < op->args.size(); i++) {
    op->args[i].accept(this);
  }
  // op->condition.accept(this);
...
void IRVisitor::visit(const Atomic *op) { op->body.accept(this); }
...
void IRGraphVisitor::visit(const Cast *op) { include(op->value); }
...
void IRGraphVisitor::visit(const ProducerConsumer *op) { include(op->body); }
...
void IRGraphVisitor::visit(const DoWhile *op) {
  include(op->min);
  include(op->cond);
  include(op->step);
  include(op->body);
...
void IRGraphVisitor::visit(const TensorOp *op) {
  for (size_t i = 0; i < op->args.size(); i++) {
    // include(op->bounds[i].min);
    include(op->args[i]);
  }
...
void IRGraphVisitor::visit(const Atomic *op) { include(op->body); }
