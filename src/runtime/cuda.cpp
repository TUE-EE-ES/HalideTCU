...
WEAK CUresult create_cuda_context(void *user_context, CUcontext *ctx) {
    ...
    #ifdef DEBUG_RUNTIME
    {
        ...
        int max_shared_mem = 0, max_constant_mem = 0, max_shared_mem_optin=0;
        ...
        struct {int *dst; CUdevice_attribute attr;} attrs[] = {
            ...
            {&max_shared_mem_optin,   CU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK_OPTIN},
        ...
        debug(user_context)
            ...
            << "      max shared memory per block optin: " << max_shared_mem_optin << "\n"
...
WEAK int halide_cuda_run(void *user_context,
    ...
    err =cuFuncSetAttribute(f,CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES, 65536);
    if (err != CUDA_SUCCESS) {
        error(user_context) << "CUDA: atribute failed: "
                            << get_error_name(err);
        return err;
    }
