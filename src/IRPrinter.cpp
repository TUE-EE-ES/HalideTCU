...
void IRPrinter::test() {
  ...
  Stmt for_loop =
      For::make("x", -2, y + 2, ForType::Parallel, DeviceAPI::Host, store);
  vector<Expr> args(1);
  args[0] = x % 3;
  Expr call = Call::make(i32, "buf", args, Call::Extern);
  Stmt store2 = Store::make("out", call + 1, x, Parameter(), const_true(),
                            ModulusRemainder(3, 5));
  Stmt for_loop2 =
      For::make("x", 0, y, ForType::Vectorized, DeviceAPI::Host, store2);
  ...
  Stmt assertion = AssertStmt::make(
      y >= 3, Call::make(Int(32), "halide_error_param_too_small_i64",
                         {string("y"), y, 3}, Call::Extern));
  Stmt block = Block::make(assertion, pipeline);
  Stmt let_stmt = LetStmt::make("y", 17, block);
  Stmt allocate = Allocate::make("buf", f32, MemoryType::Stack, {1023},
                                 const_true(), let_stmt);
...
void IRPrinter::print(Expr ir) { ir.accept(this); }
...
void IRPrinter::visit(const Variable *op) {
  if (!known_type.contains(op->name) && (op->type != Int(32))) {
    stream << '(' << op->type << ')';
  }
  stream << op->name;
}
...
void IRPrinter::visit(const Load *op) {
  ...
  print(op->index);
  if (show_alignment) {
    stream << " aligned(" << op->alignment.modulus << ", "
...
void IRPrinter::visit(const DoWhile *op) {
  ScopedBinding<> bind(known_type, op->name);
  stream << get_indent() << op->for_type << op->device_api << " (" << op->name
         << ", ";
  print(op->min);
...
void IRPrinter::visit(const Store *op) {
  stream << get_indent();
  const bool has_pred = !is_one(op->predicate);
  const bool show_alignment =
      op->value.type().is_vector() && (op->alignment.modulus > 1);
  if (has_pred) {
    stream << "predicate (";
    print(op->predicate);
    stream << ")\n";
...
void IRPrinter::visit(const Allocate *op) {
  ...
  if (!op->free_function.empty()) {
    stream << "\n";
    stream << get_indent() << " custom_delete { " << op->free_function << "("
           << op->name << "); }";
  }
  stream << "\n";
  print(op->body);
...
void IRPrinter::visit(const TensorOp *op) {
  print(op->name);
...
void IRPrinter::visit(const Shuffle *op) {
  if (op->is_concat()) {
    ...
    print_list(op->vectors);
    stream << ", " << op->slice_begin() << ", " << op->slice_stride() << ", "
           << op->indices.size() << ")";
  } else {
    stream << "shuffle(";
    print_list(op->vectors);
