...
inline HALIDE_NO_USER_CODE_INLINE void
collect_print_args(std::vector<Expr> &args) {}

template <typename... Args>
inline HALIDE_NO_USER_CODE_INLINE void
collect_print_args(std::vector<Expr> &args, const char *arg,
                   Args &&... more_args) {
...
template <typename... Args>
inline HALIDE_NO_USER_CODE_INLINE void
collect_print_args(std::vector<Expr> &args, Expr arg, Args &&... more_args) {
...
template <typename T> inline Expr cast(Expr a) {
...
inline Expr max(A &&a, B &&b, C &&c, Rest &&... rest) {
  return max(std::forward<A>(a), max(std::forward<B>(b), std::forward<C>(c),
...
inline Expr min(A &&a, B &&b, C &&c, Rest &&... rest) {
  return min(std::forward<A>(a), min(std::forward<B>(b), std::forward<C>(c),
                                     std::forward<Rest>(rest)...));
}
...
inline Expr select(Expr c0, Expr v0, Expr c1, Expr v1, Args &&... args) {
  return select(
      std::move(c0), std::move(v0),
      select(std::move(c1), std::move(v1), std::forward<Args>(args)...));
}

/** Equivalent of ternary select(), but taking/returning tuples. If the
 * condition is a Tuple, it must match the size of the true and false Tuples. */
// @{
...
/** Equivalent of multiway select(), but taking/returning tuples. If the
 * condition is a Tuple, it must match the size of the true and false Tuples. */
// @{
template <typename... Args>
inline Tuple tuple_select(const Tuple &c0, const Tuple &v0, const Tuple &c1,
                          const Tuple &v1, Args &&... args) {
  return tuple_select(c0, v0,
                      tuple_select(c1, v1, std::forward<Args>(args)...));
}

template <typename... Args>
inline Tuple tuple_select(const Expr &c0, const Tuple &v0, const Expr &c1,
                          const Tuple &v1, Args &&... args) {
  return tuple_select(c0, v0,
                      tuple_select(c1, v1, std::forward<Args>(args)...));
}
 ...
 * Float(32). Absolute approximation error is less than 1e-5. */
// @{
 ...
 * across different implementations of the same architecture (e.g. AMD vs
 * Intel), even when strict_float is enabled. */
 ...
 * ARM. Vectorizes cleanly. Note that this can produce slightly different
 * results across different implementations of the same architecture (e.g. AMD
 * vs Intel), even when strict_float is enabled. */
...
template <typename T> Expr reinterpret(Expr e) {
...
/** Count the number of trailing zero bits in an expression. If the expression
 * is zero, the result is the number of bits in the type. */
...
template <typename... Args>
inline HALIDE_NO_USER_CODE_INLINE Expr print_when(Expr condition, Expr a,
                                                  Args &&... args) {
  std::vector<Expr> collected_args = {std::move(a)};
  Internal::collect_print_args(collected_args, std::forward<Args>(args)...);
...
template <typename... Args>
inline HALIDE_NO_USER_CODE_INLINE Expr require(Expr condition, Expr value,
                                               Args &&... args) {
...
// @{
template <typename... Args>
inline HALIDE_NO_USER_CODE_INLINE Expr memoize_tag(Expr result,
                                                   Args &&... args) {
...
template <typename T> Expr saturating_cast(Expr e) {
