...
void visit(const DoWhile *op) override {
        ...
        stream << " (";
        stream << close_span();
        print_list({Variable::make(Int(32), op->name), op->min, op->cond,op->step});
        stream << matched(")");
        stream << close_expand_button();
        stream << " " << matched("{");
        stream << open_div("ForBody Indent", id);
    ...
    void visit(const TensorOp *op) override {
        stream << open_span("TensorOp");
        stream << keyword("TensorOp") << " ";
        stream << var(op->name);
        stream << matched("(");
        for (size_t i = 0; i < op->args.size(); i++) {
            print_list("[", {op->args[i]}, "]");
