...
Stmt Simplify::visit(const DoWhile *op) {
    
    Expr new_min = (op->min);
    Expr new_cond = (op->cond);

    Expr new_step = (op->step);
    Stmt new_body = mutate(op->body);
    ...
    if (is_no_op(new_body)) {
        return new_body;   
    } else if (op->min.same_as(new_min) && op->step.same_as(new_step) &&
               op->cond.same_as(new_cond) &&
               op->body.same_as(new_body)) {
        return op;
    } else {
        return DoWhile::make(op->name, new_min, new_cond,new_step, op->for_type, op->device_api, new_body);
    }
}
...
Stmt Simplify::visit(const TensorOp *op) {
    Stmt body = mutate(op->body);
    ...
    //     if(!new_args[i].same_as(op->args[i]))   args_changed=true;
    // }
    //Region new_bounds;
    //bool bounds_changed;
    ...
    return TensorOp::make(op->name,op->types,op->args, std::move(body));
}
