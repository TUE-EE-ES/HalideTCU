...
apply_outer_split(const Split &split, bool is_update, string prefix,
                  map<string, Expr> &dim_extent_alignment) {
  vector<ApplySplitResult> result;
  std::cout << "IN OUTER SPLIT"
            << "\n";
  ...
  if ((iter != dim_extent_alignment.end()) &&
    ...
    // statement.
    dim_extent_alignment[split.outer] = iter->second;
...
compute_loop_bounds_after_outer_split(Split &split, string prefix) {
  ...
  //        Expr outer_extent = (old_var_max - old_var_min );
...
vector<ApplySplitResult> apply_split(const Split &split, bool is_update,
                                     string prefix,
                                     map<string, Expr> &dim_extent_alignment) {
  vector<ApplySplitResult> result;
  ...
  if (split.is_split()) {
    ...
    if ((iter != dim_extent_alignment.end()) &&
      ...
      Expr cond = likely(rebased_var < old_extent);
      result.push_back(ApplySplitResult(cond));
      result.push_back(ApplySplitResult(rebased_var_name, rebased,
    ...
    // Substitute in the new expression for the split variable ...
    result.push_back(ApplySplitResult(old_var_name, base_var + inner,
                                      ApplySplitResult::Substitution));
    // ... but also define it as a let for the benefit of bounds inference.
    result.push_back(ApplySplitResult(old_var_name, base_var + inner,
                                      ApplySplitResult::LetStmt));
    ...
    Expr inner_min =
        Variable::make(Int(32), prefix + split.inner + ".loop_min");
    Expr outer_min =
        Variable::make(Int(32), prefix + split.outer + ".loop_min");
    Expr inner_extent =
        Variable::make(Int(32), prefix + split.inner + ".loop_extent");
    ...
    result.push_back(ApplySplitResult(prefix + split.inner, inner,
                                      ApplySplitResult::Substitution));
    result.push_back(ApplySplitResult(prefix + split.outer, outer,
                                      ApplySplitResult::Substitution));
    result.push_back(ApplySplitResult(prefix + split.inner, inner,
                                      ApplySplitResult::LetStmt));
    result.push_back(ApplySplitResult(prefix + split.outer, outer,
    ...
    map<string, Expr>::iterator inner_dim =
        dim_extent_alignment.find(split.inner);
    map<string, Expr>::iterator outer_dim =
        dim_extent_alignment.find(split.outer);
    if (inner_dim != dim_extent_alignment.end() &&
        outer_dim != dim_extent_alignment.end()) {
      dim_extent_alignment[split.old_var] =
          inner_dim->second * outer_dim->second;
    }
  } else {
    // rename or purify
    result.push_back(ApplySplitResult(prefix + split.old_var, outer,
                                      ApplySplitResult::Substitution));
    result.push_back(ApplySplitResult(prefix + split.old_var, outer,
                                      ApplySplitResult::LetStmt));
  }
...
vector<std::pair<string, Expr>>
compute_loop_bounds_after_split(const Split &split, string prefix) {
  // Define the bounds on the split dimensions using the bounds
  // on the function args. If it is a purify, we should use the bounds
  // from the dims instead.
  ...
  if (split.is_split()) {
    Expr inner_extent = split.factor;
    Expr outer_extent =
        (old_var_max - old_var_min + split.factor) / split.factor;
    ...
    Expr inner_extent =
        Variable::make(Int(32), prefix + split.inner + ".loop_extent");
    Expr outer_extent =
        Variable::make(Int(32), prefix + split.outer + ".loop_extent");
    Expr fused_extent = inner_extent * outer_extent;
    let_stmts.push_back({prefix + split.old_var + ".loop_min", 0});
    let_stmts.push_back(
        {prefix + split.old_var + ".loop_max", fused_extent - 1});
    let_stmts.push_back(
        {prefix + split.old_var + ".loop_extent", fused_extent});
    ...
    let_stmts.push_back(
        {prefix + split.outer + ".loop_extent", old_var_extent});
  }
  // Do nothing for purify
