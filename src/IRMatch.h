...
/** \file
 * Defines a method to match a fragment of IR against a pattern containing
 * wildcards
 */
...
struct MatcherState {
  ...
  void set_binding(int i, const BaseExprNode &n) noexcept { bindings[i] = &n; }

  HALIDE_ALWAYS_INLINE
  const BaseExprNode *get_binding(int i) const noexcept { return bindings[i]; }
  ...
  HALIDE_ALWAYS_INLINE
  void set_bound_const(int i, halide_scalar_value_t val,
                       halide_type_t t) noexcept {
    bound_const[i] = val;
    bound_const_type[i] = t;
  }

  HALIDE_ALWAYS_INLINE
  void get_bound_const(int i, halide_scalar_value_t &val,
                       halide_type_t &type) const noexcept {
    val = bound_const[i];
...
// A fast version of expression equality that assumes a well-typed non-null
// expression tree.
HALIDE_ALWAYS_INLINE
bool equal(const BaseExprNode &a, const BaseExprNode &b) noexcept {
  // Early out
  return (&a == &b) || ((a.type == b.type) && (a.node_type == b.node_type) &&
                        equal_helper(a, b));
}
...
struct SpecificExpr {
  ...
  HALIDE_ALWAYS_INLINE bool match(const BaseExprNode &e,
                                  MatcherState &state) const noexcept {
    return equal(*expr.get(), e);
  }

  HALIDE_ALWAYS_INLINE
  Expr make(MatcherState &state, halide_type_t type_hint) const { return expr; }
...
template <int i> struct WildConstInt {
  struct pattern_tag {};
  ...
  HALIDE_ALWAYS_INLINE
  void make_folded_const(halide_scalar_value_t &val, halide_type_t &ty,
...
template <int i> struct WildConstUInt {
  struct pattern_tag {};
...
template <int i> struct WildConstFloat {
  struct pattern_tag {};
...
template <int i> struct WildConst {
  struct pattern_tag {};
...
template <int i> std::ostream &operator<<(std::ostream &s, const Wild<i> &op) {
  s << "_" << i;
  return s;
}
...
struct Const {
  ...
  template <uint32_t bound>
  HALIDE_ALWAYS_INLINE bool match(const Const &b, MatcherState &state) const
      noexcept {
    return v == b.v;
  }
...
HALIDE_ALWAYS_INLINE
Const pattern_arg(int64_t x) { return {x}; }
HALIDE_ALWAYS_INLINE
const SpecificExpr pattern_arg(const Expr &e) { return {e}; }
...
template <typename T,
          ...
          typename = typename std::enable_if<!std::is_same<
              typename std::decay<T>::type, SpecificExpr>::value>::type>
HALIDE_ALWAYS_INLINE T unwrap(T t) {
  return t;
}

HALIDE_ALWAYS_INLINE
const BaseExprNode &unwrap(const SpecificExpr &e) { return *e.expr.get(); }
...
template <typename Op, typename A, typename B> struct BinOp {
  struct pattern_tag {};
  A a;
  B b;
  ...
  HALIDE_ALWAYS_INLINE bool match(const BinOp<Op2, A2, B2> &op,
                                  MatcherState &state) const noexcept {
...
template <typename Op> uint64_t constant_fold_cmp_op(int64_t, int64_t) noexcept;

template <typename Op>
uint64_t constant_fold_cmp_op(uint64_t, uint64_t) noexcept;

template <typename Op> uint64_t constant_fold_cmp_op(double, double) noexcept;

// Matches one of the comparison operators
template <typename Op, typename A, typename B> struct CmpOp {
  struct pattern_tag {};
  A a;
  B b;
  ...
  template <uint32_t bound, typename Op2, typename A2, typename B2>
  HALIDE_ALWAYS_INLINE bool match(const CmpOp<Op2, A2, B2> &op,
                                  MatcherState &state) const noexcept {
    return (std::is_same<Op, Op2>::value &&
            a.template match<bound>(unwrap(op.a), state) &&
            b.template match<bound | bindings<A>::mask>(unwrap(op.b), state));
  ...
  void make_folded_const(halide_scalar_value_t &val, halide_type_t &ty,
                         MatcherState &state) const noexcept {
    halide_scalar_value_t val_a, val_b;
    // If one side is an untyped const, evaluate the other side first to get a
    // type hint.
    if (std::is_same<A, Const>::value) {
      b.make_folded_const(val_b, ty, state);
      const uint16_t l = ty.lanes;
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto operator+(A a, B b) noexcept
    -> BinOp<Add, decltype(pattern_arg(a)), decltype(pattern_arg(b))> {
  return {pattern_arg(a), pattern_arg(b)};
}

template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto add(A a, B b)
    -> decltype(IRMatcher::operator+(a, b)) {
  return IRMatcher::operator+(a, b);
}

template <>
HALIDE_ALWAYS_INLINE int64_t constant_fold_bin_op<Add>(halide_type_t &t,
                                                       int64_t a,
                                                       int64_t b) noexcept {
  t.lanes |= ((t.bits >= 32) && add_would_overflow(t.bits, a, b))
                 ? MatcherState::signed_integer_overflow
                 : 0;
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_bin_op<Add>(halide_type_t &t,
                                                        uint64_t a,
                                                        uint64_t b) noexcept {
...
template <>
HALIDE_ALWAYS_INLINE double
constant_fold_bin_op<Add>(halide_type_t &t, double a, double b) noexcept {
  return a + b;
}
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto sub(A a, B b)
    -> decltype(IRMatcher::operator-(a, b)) {
  return IRMatcher::operator-(a, b);
}

template <>
HALIDE_ALWAYS_INLINE int64_t constant_fold_bin_op<Sub>(halide_type_t &t,
                                                       int64_t a,
                                                       int64_t b) noexcept {
  t.lanes |= ((t.bits >= 32) && sub_would_overflow(t.bits, a, b))
                 ? MatcherState::signed_integer_overflow
                 : 0;
  // Drop the high bits then sign-extend them back
  int dead_bits = 64 - t.bits;
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_bin_op<Sub>(halide_type_t &t,
                                                        uint64_t a,
                                                        uint64_t b) noexcept {
  uint64_t ones = (uint64_t)(-1);
  return (a - b) & (ones >> (64 - t.bits));
...
template <>
HALIDE_ALWAYS_INLINE double
constant_fold_bin_op<Sub>(halide_type_t &t, double a, double b) noexcept {
  return a - b;
}
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto mul(A a, B b)
    -> decltype(IRMatcher::operator*(a, b)) {
  return IRMatcher::operator*(a, b);
}

template <>
HALIDE_ALWAYS_INLINE int64_t constant_fold_bin_op<Mul>(halide_type_t &t,
                                                       int64_t a,
                                                       int64_t b) noexcept {
  t.lanes |= ((t.bits >= 32) && mul_would_overflow(t.bits, a, b))
                 ? MatcherState::signed_integer_overflow
                 : 0;
  int dead_bits = 64 - t.bits;
  // Drop the high bits then sign-extend them back
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_bin_op<Mul>(halide_type_t &t,
                                                        uint64_t a,
                                                        uint64_t b) noexcept {
  uint64_t ones = (uint64_t)(-1);
  return (a * b) & (ones >> (64 - t.bits));
...
template <>
HALIDE_ALWAYS_INLINE double
constant_fold_bin_op<Mul>(halide_type_t &t, double a, double b) noexcept {
  return a * b;
}
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto div(A a, B b)
    -> decltype(IRMatcher::operator/(a, b)) {
  return IRMatcher::operator/(a, b);
}

template <>
HALIDE_ALWAYS_INLINE int64_t constant_fold_bin_op<Div>(halide_type_t &t,
                                                       int64_t a,
                                                       int64_t b) noexcept {
  if (b == 0) {
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_bin_op<Div>(halide_type_t &t,
                                                        uint64_t a,
                                                        uint64_t b) noexcept {
  if (b == 0) {
    t.lanes |= MatcherState::indeterminate_expression;
...
template <>
HALIDE_ALWAYS_INLINE double
constant_fold_bin_op<Div>(halide_type_t &t, double a, double b) noexcept {
  return a / b;
}
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto mod(A a, B b)
    -> decltype(IRMatcher::operator%(a, b)) {
  return IRMatcher::operator%(a, b);
}

template <>
HALIDE_ALWAYS_INLINE int64_t constant_fold_bin_op<Mod>(halide_type_t &t,
                                                       int64_t a,
                                                       int64_t b) noexcept {
  if (b == 0) {
    t.lanes |= MatcherState::indeterminate_expression;
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_bin_op<Mod>(halide_type_t &t,
                                                        uint64_t a,
                                                        uint64_t b) noexcept {
  if (b == 0) {
    t.lanes |= MatcherState::indeterminate_expression;
...
template <>
HALIDE_ALWAYS_INLINE double
constant_fold_bin_op<Mod>(halide_type_t &t, double a, double b) noexcept {
  return mod_imp(a, b);
}

template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto min(A a, B b) noexcept
    -> BinOp<Min, decltype(pattern_arg(a)), decltype(pattern_arg(b))> {
  return {pattern_arg(a), pattern_arg(b)};
}

template <>
HALIDE_ALWAYS_INLINE int64_t constant_fold_bin_op<Min>(halide_type_t &t,
                                                       int64_t a,
                                                       int64_t b) noexcept {
  return std::min(a, b);
}
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_bin_op<Min>(halide_type_t &t,
                                                        uint64_t a,
                                                        uint64_t b) noexcept {
  return std::min(a, b);
}
...
template <>
HALIDE_ALWAYS_INLINE double
constant_fold_bin_op<Min>(halide_type_t &t, double a, double b) noexcept {
  return std::min(a, b);
}

template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto max(A a, B b) noexcept
    -> BinOp<Max, decltype(pattern_arg(a)), decltype(pattern_arg(b))> {
  return {pattern_arg(a), pattern_arg(b)};
}

template <>
HALIDE_ALWAYS_INLINE int64_t constant_fold_bin_op<Max>(halide_type_t &t,
                                                       int64_t a,
                                                       int64_t b) noexcept {
  return std::max(a, b);
}
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_bin_op<Max>(halide_type_t &t,
                                                        uint64_t a,
                                                        uint64_t b) noexcept {
  return std::max(a, b);
}
...
template <>
HALIDE_ALWAYS_INLINE double
constant_fold_bin_op<Max>(halide_type_t &t, double a, double b) noexcept {
  return std::max(a, b);
}
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<LT>(int64_t a,
                                                       int64_t b) noexcept {
  return a < b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<LT>(uint64_t a,
                                                       uint64_t b) noexcept {
  return a < b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<LT>(double a,
                                                       double b) noexcept {
  return a < b;
}
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<GT>(int64_t a,
                                                       int64_t b) noexcept {
  return a > b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<GT>(uint64_t a,
                                                       uint64_t b) noexcept {
  return a > b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<GT>(double a,
                                                       double b) noexcept {
  return a > b;
}
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto le(A a, B b)
    -> decltype(IRMatcher::operator<=(a, b)) {
  return IRMatcher::operator<=(a, b);
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<LE>(int64_t a,
                                                       int64_t b) noexcept {
  return a <= b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<LE>(uint64_t a,
                                                       uint64_t b) noexcept {
  return a <= b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<LE>(double a,
                                                       double b) noexcept {
  return a <= b;
}
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto ge(A a, B b)
    -> decltype(IRMatcher::operator>=(a, b)) {
  return IRMatcher::operator>=(a, b);
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<GE>(int64_t a,
                                                       int64_t b) noexcept {
  return a >= b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<GE>(uint64_t a,
                                                       uint64_t b) noexcept {
  return a >= b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<GE>(double a,
                                                       double b) noexcept {
  return a >= b;
}
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto eq(A a, B b)
    -> decltype(IRMatcher::operator==(a, b)) {
  return IRMatcher::operator==(a, b);
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<EQ>(int64_t a,
                                                       int64_t b) noexcept {
  return a == b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<EQ>(uint64_t a,
                                                       uint64_t b) noexcept {
  return a == b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<EQ>(double a,
                                                       double b) noexcept {
  return a == b;
}
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto ne(A a, B b)
    -> decltype(IRMatcher::operator!=(a, b)) {
  return IRMatcher::operator!=(a, b);
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<NE>(int64_t a,
                                                       int64_t b) noexcept {
  return a != b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<NE>(uint64_t a,
                                                       uint64_t b) noexcept {
  return a != b;
}

template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_cmp_op<NE>(double a,
                                                       double b) noexcept {
  return a != b;
}
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto or_op(A a, B b)
    -> decltype(IRMatcher::operator||(a, b)) {
  return IRMatcher::operator||(a, b);
}

template <>
HALIDE_ALWAYS_INLINE int64_t constant_fold_bin_op<Or>(halide_type_t &t,
                                                      int64_t a,
                                                      int64_t b) noexcept {
  return (a | b) & 1;
}
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_bin_op<Or>(halide_type_t &t,
                                                       uint64_t a,
                                                       uint64_t b) noexcept {
  return (a | b) & 1;
}
...
template <>
HALIDE_ALWAYS_INLINE double constant_fold_bin_op<Or>(halide_type_t &t, double a,
                                                     double b) noexcept {
  t.lanes |= MatcherState::indeterminate_expression;
  return 0;
}
...
template <typename A, typename B>
HALIDE_ALWAYS_INLINE auto and_op(A a, B b)
    -> decltype(IRMatcher::operator&&(a, b)) {
  return IRMatcher::operator&&(a, b);
}

template <>
HALIDE_ALWAYS_INLINE int64_t constant_fold_bin_op<And>(halide_type_t &t,
                                                       int64_t a,
                                                       int64_t b) noexcept {
  return a & b & 1;
}
...
template <>
HALIDE_ALWAYS_INLINE uint64_t constant_fold_bin_op<And>(halide_type_t &t,
                                                        uint64_t a,
                                                        uint64_t b) noexcept {
  return a & b & 1;
}
...
template <>
HALIDE_ALWAYS_INLINE double
constant_fold_bin_op<And>(halide_type_t &t, double a, double b) noexcept {
  t.lanes |= MatcherState::indeterminate_expression;
  return 0;
}
...
template <typename... Args> struct Intrin {
  struct pattern_tag {};
  Call::IntrinsicOp intrin;
  std::tuple<Args...> args;

  static constexpr uint32_t binds =
      bitwise_or_reduce((bindings<Args>::mask)...);

  template <int i, uint32_t bound,
  ...
  HALIDE_ALWAYS_INLINE
  void print_args(std::ostream &s) const { print_args<0>(0, s); }
...
template <typename... Args>
HALIDE_ALWAYS_INLINE auto intrin(Call::IntrinsicOp intrinsic_op,
                                 Args... args) noexcept
    -> Intrin<decltype(pattern_arg(args))...> {
  return {intrinsic_op, pattern_arg(args)...};
}

template <typename A> struct NotOp {
  struct pattern_tag {};
  A a;
  ...
  HALIDE_ALWAYS_INLINE bool match(const NotOp<A2> &op,
                                  MatcherState &state) const noexcept {
  ...
  HALIDE_ALWAYS_INLINE void make_folded_const(halide_scalar_value_t &val,
                                              halide_type_t &ty,
                                              MatcherState &state) const
      noexcept {
...
template <typename A>
HALIDE_ALWAYS_INLINE auto operator!(A a) noexcept
    -> NotOp<decltype(pattern_arg(a))> {
...
template <typename C, typename T, typename F> struct SelectOp {
  struct pattern_tag {};
  C c;
  T t;
  F f;
  ...
  constexpr static uint32_t binds =
      bindings<C>::mask | bindings<T>::mask | bindings<F>::mask;
  ...
  HALIDE_ALWAYS_INLINE bool match(const BaseExprNode &e,
    ...
    return (c.template match<bound>(*op.condition.get(), state) &&
            t.template match<bound | bindings<C>::mask>(*op.true_value.get(),
                                                        state) &&
            f.template match<bound | bindings<C>::mask | bindings<T>::mask>(
                *op.false_value.get(), state));
  }
  ...
  HALIDE_ALWAYS_INLINE bool match(const SelectOp<C2, T2, F2> &instance,
    ...
    return (c.template match<bound>(unwrap(instance.c), state) &&
            t.template match<bound | bindings<C>::mask>(unwrap(instance.t),
                                                        state) &&
            f.template match<bound | bindings<C>::mask | bindings<T>::mask>(
                unwrap(instance.f), state));
  }

  HALIDE_ALWAYS_INLINE
  Expr make(MatcherState &state, halide_type_t type_hint) const {
    return Select::make(c.make(state, {}), t.make(state, type_hint),
...
template <typename C, typename T, typename F>
HALIDE_ALWAYS_INLINE auto select(C c, T t, F f) noexcept
    -> SelectOp<decltype(pattern_arg(c)), decltype(pattern_arg(t)),
                decltype(pattern_arg(f))> {
  return {pattern_arg(c), pattern_arg(t), pattern_arg(f)};
}

template <typename A, bool known_lanes> struct BroadcastOp {
  struct pattern_tag {};
  A a;
  int lanes;
  ...
  HALIDE_ALWAYS_INLINE bool match(const BroadcastOp<A2, known_lanes_2> &op,
                                  MatcherState &state) const noexcept {
    return (a.template match<bound>(unwrap(op.a), state) &&
...
template <typename A>
HALIDE_ALWAYS_INLINE auto broadcast(A a, int lanes) noexcept
    -> BroadcastOp<decltype(pattern_arg(a)), true> {
  return {pattern_arg(a), lanes};
}

template <typename A>
HALIDE_ALWAYS_INLINE auto broadcast(A a) noexcept
    -> BroadcastOp<decltype(pattern_arg(a)), false> {
  return {pattern_arg(a), 0};
}

template <typename A, typename B, bool known_lanes> struct RampOp {
  struct pattern_tag {};
  A a;
  B b;
  int lanes;
  ...
  HALIDE_ALWAYS_INLINE bool match(const RampOp<A2, B2, known_lanes_2> &op,
                                  MatcherState &state) const noexcept {
...
template <typename A> struct NegateOp {
  ...
  HALIDE_ALWAYS_INLINE bool match(const BaseExprNode &e,
    ...
    return (a.template match<bound>(*op.b.get(), state) && is_zero(op.a));
  }

  template <uint32_t bound, typename A2>
  HALIDE_ALWAYS_INLINE bool match(NegateOp<A2> &&p, MatcherState &state) const
      noexcept {
...
template <typename A>
HALIDE_ALWAYS_INLINE auto operator-(A a) noexcept
    -> NegateOp<decltype(pattern_arg(a))> {
  return {pattern_arg(a)};
}
...
template <typename A> struct CastOp {
  struct pattern_tag {};
  Type t;
  A a;
  ...
  HALIDE_ALWAYS_INLINE bool match(const BaseExprNode &e,
    ...
    return (e.type == t && a.template match<bound>(*op.value.get(), state));
  }
  template <uint32_t bound, typename A2>
  HALIDE_ALWAYS_INLINE bool match(const CastOp<A2> &op,
                                  MatcherState &state) const noexcept {
...
template <typename A>
HALIDE_ALWAYS_INLINE auto cast(halide_type_t t, A a) noexcept
    -> CastOp<decltype(pattern_arg(a))> {
  return {t, pattern_arg(a)};
}

template <typename A> struct Fold {
  struct pattern_tag {};
  A a;
...
template <typename A> struct Overflows {
  struct pattern_tag {};
  A a;
...
template <typename A>
HALIDE_ALWAYS_INLINE auto overflows(A a) noexcept
    -> Overflows<decltype(pattern_arg(a))> {
  return {pattern_arg(a)};
}
...
template <typename A> struct IsConst {
  struct pattern_tag {};
...
template <typename A>
HALIDE_ALWAYS_INLINE auto is_const(A a) noexcept
...
template <typename A, typename Prover> struct CanProve {
  struct pattern_tag {};
  A a;
  ...
  HALIDE_NEVER_INLINE void make_folded_const(halide_scalar_value_t &val,
                                             halide_type_t &ty,
                                             MatcherState &state) const {
...
HALIDE_ALWAYS_INLINE auto can_prove(A a, Prover *p) noexcept
...
template <typename A> struct IsFloat {
  struct pattern_tag {};
  A a;
  ...
  void make_folded_const(halide_scalar_value_t &val, halide_type_t &ty,
                         MatcherState &state) const {
    // a is almost certainly a very simple pattern (e.g. a wild), so just inline
    // the make method.
...
template <typename A>
HALIDE_ALWAYS_INLINE auto is_float(A a) noexcept
    -> IsFloat<decltype(pattern_arg(a))> {
  return {pattern_arg(a)};
}
...
template <
    typename Before, typename After, typename Predicate,
    typename = typename std::enable_if<std::decay<Before>::type::foldable &&
                                       std::decay<After>::type::foldable>::type>
HALIDE_NEVER_INLINE void fuzz_test_rule(Before &&before, After &&after,
                                        Predicate &&pred,
                                        halide_type_t wildcard_type,
                                        halide_type_t output_type) noexcept {
  ...
  debug(0) << "validate('" << before << "', '" << after << "', '" << pred
           << "', " << Type(wildcard_type) << ", " << Type(output_type)
           << ")\n";
  ...
  for (int trials = 0; trials < 100; trials++) {
    ...
    for (int i = 0; i < max_wild; i++) {
      ...
      case halide_type_uint: {
        ...
        uint64_t val = constant_fold_bin_op<Add>(wildcard_type,
                                                 (uint64_t)rng() >> shift, 0);
        state.set_bound_const(i, val, wildcard_type);
        val = constant_fold_bin_op<Add>(wildcard_type, (uint64_t)rng() >> shift,
                                        0);
        exprs[i] = make_const(wildcard_type, val);
        state.set_binding(i, *exprs[i].get());
      } break;
      case halide_type_int: {
        int64_t val = constant_fold_bin_op<Add>(wildcard_type,
                                                (int64_t)rng() >> shift, 0);
        state.set_bound_const(i, val, wildcard_type);
        val = constant_fold_bin_op<Add>(wildcard_type, (int64_t)rng() >> shift,
                                        0);
        exprs[i] = make_const(wildcard_type, val);
      } break;
      case halide_type_float:
    ...
    case halide_type_bfloat: {
      ...
      ok &= (error < 0.01 || val_before.u.u64 == val_after.u.u64 ||
...
template <typename Before, typename After, typename Predicate,
          typename = typename std::enable_if<
              !(std::decay<Before>::type::foldable &&
                std::decay<After>::type::foldable)>::type>
HALIDE_ALWAYS_INLINE void
fuzz_test_rule(Before &&before, After &&after, Predicate &&pred, halide_type_t,
               halide_type_t, int dummy = 0) noexcept {
  // We can't verify rewrite rules that can't be constant-folded.
}

HALIDE_ALWAYS_INLINE
bool evaluate_predicate(bool x, MatcherState &) noexcept { return x; }
...
HALIDE_ALWAYS_INLINE bool evaluate_predicate(Pattern p, MatcherState &state) {
  ...
  return (c.u.u64 != 0) &&
         ((ty.lanes & MatcherState::special_values_mask) == 0);
}
...
template <typename Instance> struct Rewriter {
  Instance instance;
  Expr result;
  MatcherState state;
  ...
  Rewriter(Instance &&instance, halide_type_t ot, halide_type_t wt)
      : instance(std::forward<Instance>(instance)), output_type(ot),
        wildcard_type(wt) {}
  ...
  HALIDE_ALWAYS_INLINE bool operator()(Before before, After after) {
    static_assert((Before::binds & After::binds) == After::binds,
  ...
  template <typename Before,
            typename = typename enable_if_pattern<Before>::type>
  HALIDE_ALWAYS_INLINE bool operator()(Before before,
                                       const Expr &after) noexcept {
    if (before.template match<0>(instance, state)) {
      result = after;
#if HALIDE_DEBUG_MATCHED_RULES
      debug(0) << instance << " -> " << result << " via " << before << " -> "
  ...
  HALIDE_ALWAYS_INLINE bool operator()(Before before, After after,
    ...
    static_assert((Before::binds & After::binds) == After::binds,
                  "Rule result uses unbound values");
    static_assert((Before::binds & Predicate::binds) == Predicate::binds,
  ...
  template <typename Before, typename Predicate,
            typename = typename enable_if_pattern<Before>::type,
            typename = typename enable_if_pattern<Predicate>::type>
  HALIDE_ALWAYS_INLINE bool operator()(Before before, const Expr &after,
                                       Predicate pred) {
    static_assert(
        Predicate::foldable,
        "Predicates must consist only of operations that can constant-fold");
  ...
  template <typename Before, typename Predicate,
            typename = typename enable_if_pattern<Before>::type,
            typename = typename enable_if_pattern<Predicate>::type>
  HALIDE_ALWAYS_INLINE bool operator()(Before before, int64_t after,
                                       Predicate pred) {
    static_assert(
        Predicate::foldable,
        "Predicates must consist only of operations that can constant-fold");
...
HALIDE_ALWAYS_INLINE auto rewriter(Instance instance, halide_type_t output_type,
                                   halide_type_t wildcard_type) noexcept
...
template <typename Instance,
          typename = typename enable_if_pattern<Instance>::type>
HALIDE_ALWAYS_INLINE auto rewriter(Instance instance,
                                   halide_type_t output_type) noexcept
    -> Rewriter<decltype(pattern_arg(instance))> {
...
HALIDE_ALWAYS_INLINE
auto rewriter(const Expr &e, halide_type_t wildcard_type) noexcept
