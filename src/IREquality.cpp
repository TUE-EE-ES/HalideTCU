...
private:
  ...
  CmpResult compare_expr_vector(const std::vector<Expr> &a,
...
IRComparer::CmpResult IRComparer::compare_types(Type a, Type b) {
  ...
  for (size_t i = 0; i < ha->enclosing_types.size(); i++) {
    ...
    compare_names(ha->enclosing_types[i].name, hb->enclosing_types[i].name);
  }
  ...
  for (size_t i = 0; i < ha->cpp_type_modifiers.size(); i++) {
    compare_scalar(ha->cpp_type_modifiers[i], hb->cpp_type_modifiers[i]);
  }
...
IRComparer::CmpResult IRComparer::compare_expr_vector(const vector<Expr> &a,
                                                      const vector<Expr> &b) {
  if (result != Equal)
    return result;
...
void IRComparer::visit(const DoWhile *op) {
  const DoWhile *s = stmt.as<DoWhile>();

  compare_names(s->name, op->name);
  compare_scalar(s->for_type, op->for_type);
  compare_expr(s->min, op->min);
  compare_expr(s->cond, op->cond);
  compare_expr(s->step, op->step);
  compare_stmt(s->body, op->body);
}
...
void IRComparer::visit(const TensorOp *op) {
  const TensorOp *s = stmt.as<TensorOp>();
