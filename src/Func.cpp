...
const string &Func::name() const { return func.name(); }
...
Expr Func::value() const {
  user_assert(defined())
      << "Can't call Func::value() on an undefined Func. To check if a Func is "
         "defined, call Func::defined()\n";
  user_assert(func.outputs() == 1)
      << "Can't call Func::value() on Func \"" << name()
...
Tuple Func::values() const {
  user_assert(defined())
      << "Can't call Func::values() on an undefined Func. To check if a Func "
         "is defined, call Func::defined().\n";
...
const std::vector<Expr> &Func::update_args(int idx) const {
  user_assert(has_update_definition())
      ...
      << "Use Func::has_update_definition() to check for the existence of an "
         "update definition.\n";
...
Expr Func::update_value(int idx) const {
  ...
  user_assert(func.update(idx).values().size() == 1)
      << "Can't call Func::update_value() on Func \"" << name()
      << "\", because it has multiple values.\n";
...
vector<RVar> Func::rvars(int idx) const {
  ...
  const std::vector<ReductionVariable> rvars =
      func.update(idx).schedule().rvars();
...
bool Func::is_extern() const { return func.has_extern_definition(); }
...
int Func::outputs() const { return func.outputs(); }
...
std::pair<int, int> Func::add_implicit_vars(vector<Var> &args) const {
  ...
  if (iter != args.end()) {
    ...
    while ((int)args.size() < dimensions()) {
      Internal::debug(2) << "Adding implicit var " << i << " to call to "
                         << name() << "\n";
  ...
  if (defined() && args.size() != (size_t)dimensions()) {
    user_error << "Func \"" << name() << "\" was called with " << args.size()
               << " arguments, but was defined with " << dimensions() << "\n";
  }
  ...
  for (size_t i = 0; i < dims.size(); i++) {
    if (var_name_match(dims[i].var, var.name())) {
      ...
      if (!dims[i].is_pure() && var.is_rvar && is_parallel(t)) {
        if (!definition.schedule().allow_race_conditions() &&
          ...
          if (!definition.schedule().override_atomic_associativity_test()) {
            ...
            // Check whether the operator is associative and determine the
            // operator and its identity for each value in the definition if it
            // is a Tuple
            const auto &prover_result =
                prove_associativity(func_name, args, values);
      ...
      user_assert(dims[i].for_type != ForType::Vectorized)
          << "In schedule for " << name() << ", can't vectorize across "
          << var.name() << " because Func is already vectorized across "
          << dims[i].var << "\n";
    }
  }

  if (!found) {
    user_error << "In schedule for " << name() << ", could not find dimension "
               << var.name() << " to mark as " << t << " in vars for function\n"
               << dump_argument_list();
  }
}
...
void Stage::set_dim_device_api(VarOrRVar var, DeviceAPI device_api) {
  ...
  if (!found) {
    user_error << "In schedule for " << name() << ", could not find dimension "
               << var.name() << " to set to device API "
               << static_cast<int>(device_api) << " in vars for function\n"
               << dump_argument_list();
  }
}
 ...
 */
Expr substitute_self_reference(Expr val, const string &func,
                               const Function &substitute,
                               const vector<Var> &new_args) {
...
void substitute_var_in_exprs(const string &name, Expr value,
                             vector<Expr> &exprs) {
  for (auto &expr : exprs) {
...
bool apply_split(const Split &s, vector<ReductionVariable> &rvars,
  ...
  if (it != rvars.end()) {
    debug(4) << "  Splitting " << it->var << " into " << s.outer << " and "
             << s.inner << "\n";
    ...
    rvars.insert(
        it + 1, {s.outer, 0, simplify((old_extent - 1 + s.factor) / s.factor)});

    vector<ApplySplitResult> splits_result =
        apply_split(s, true, "", dim_extent_alignment);
    vector<pair<string, Expr>> bounds_let_stmts =
        compute_loop_bounds_after_split(s, "");
    apply_split_result(bounds_let_stmts, splits_result, predicates, args,
                       values);
 ...
 * new predicates corresponding to the TailStrategy to the RDom predicate list.
 */
bool apply_outer_split(Split s, vector<ReductionVariable> &rvars,
                       vector<Expr> &predicates, vector<Expr> &args,
                       vector<Expr> &values,
                       map<string, Expr> &dim_extent_alignment) {
  internal_assert(s.is_split());
  ...
  if (it != rvars.end()) {
    ...
    rvars.insert(it + 1, {s.outer, 0, simplify((old_extent))});

    vector<ApplySplitResult> splits_result =
        apply_outer_split(s, true, "", dim_extent_alignment);
    vector<pair<string, Expr>> bounds_let_stmts =
        compute_loop_bounds_after_outer_split(s, "");
    apply_split_result(bounds_let_stmts, splits_result, predicates, args,
                       values);
...
bool apply_fuse(const Split &s, vector<ReductionVariable> &rvars,
  ...
  if ((iter_outer != rvars.end()) && (iter_inner != rvars.end())) {
    debug(4) << "  Fusing " << s.outer << " and " << s.inner << " into "
             << s.old_var << "\n";
...
bool apply_split_directive(const Split &s, vector<ReductionVariable> &rvars,
  ...
  for (const ReductionVariable &rv : rvars) {
    ...
    rvar_bounds.push_back(
        {rv.var + ".loop_max", simplify(rv.min + rv.extent - 1)});
  ...
  if (s.is_split()) {
    found =
        apply_split(s, rvars, predicates, args, values, dim_extent_alignment);
  } else if (s.is_fuse()) {
    found =
        apply_fuse(s, rvars, predicates, args, values, dim_extent_alignment);
  } else if (s.is_purify()) {
    found =
        apply_purify(s, rvars, predicates, args, values, dim_extent_alignment);
  } else {
    found =
        apply_rename(s, rvars, predicates, args, values, dim_extent_alignment);
  }
...
bool apply_outer_split_directive(Split s, vector<Dim> &dims,
  ...
  for (const Bound &i : bounds) {
    // ReductionVariable rvar;
    std::cout << "new rvar " << i.var << std::endl;
    ReductionVariable rvar;
    rvar.var = i.var;
    rvar.min = 0;
  ...
  found = apply_outer_split(s, rvars, predicates, args, values,
...
bool apply_outer_split_directive(Split s, vector<ReductionVariable> &rvars,
                                 vector<Expr> &predicates, vector<Expr> &args,
                                 vector<Expr> &values) {
  map<string, Expr> dim_extent_alignment;
  for (const ReductionVariable &rv : rvars) {
...
Func Stage::rfactor(RVar r, Var v) { return rfactor({{r, v}}); }

Func Stage::rfactor(vector<pair<RVar, Var>> preserved) {
  user_assert(!definition.is_init())
      << "rfactor() must be called on an update definition\n";
  ...
  for (const pair<RVar, Var> &i : preserved) {
    ...
    const Var &v = i.second;
    {
      ...
      const auto &iter =
          std::find_if(dims.begin(), dims.end(), [&rv](const Dim &dim) {
            return var_name_match(dim.var, rv.name());
          });
      user_assert((iter != dims.end()) && (*iter).is_rvar())
          << "In schedule for " << name() << ", can't perform rfactor() on "
          << rv.name() << " since it is not in the reduction domain\n"
          << dump_argument_list();
      is_rfactored[iter - dims.begin()] = true;
    }
    {
      // Check that the new pure Vars we used to rename the RVar aren't already
      // in the dims list
      const auto &iter =
          std::find_if(dims.begin(), dims.end(), [&v](const Dim &dim) {
            return var_name_match(dim.var, v.name());
          });
      user_assert(iter == dims.end())
          << "In schedule for " << name() << ", can't rename the rvars "
          << rv.name() << " into " << v.name()
  ...
  // correctly lift the RVars not in 'rvars_kept' and distribute the RVars to
  // the intermediate and merge Funcs.
  {
    vector<Split> temp;
  ...
  for (const auto &rv : rvars) {
    const auto &iter = std::find_if(
        preserved.begin(), preserved.end(), [&rv](const pair<RVar, Var> &pair) {
  ...
  std::sort(
      ...
      [&](const pair<RVar, Var> &lhs, const pair<RVar, Var> &rhs) {
        const auto &iter_lhs = std::find_if(
            rvars.begin(), rvars.end(), [&lhs](const ReductionVariable &rv) {
              return var_name_match(rv.var, lhs.first.name());
            });
        const auto &iter_rhs = std::find_if(
            rvars.begin(), rvars.end(), [&rhs](const ReductionVariable &rv) {
  ...
  // List of pure Vars to replace the RVars in the intermediate's update
  // definition
  ...
  // are removed from the RDom
  {
    ...
    for (const auto &rv : rvars) {
      const auto &iter = std::find_if(
          rvars_kept.begin(), rvars_kept.end(), [&rv](const RVar &rvar) {
            return var_name_match(rv.var, rvar.name());
          });
  ...
  for (size_t i = 0; i < vars_rename.size(); i++) {
    ...
    const auto &iter = std::find_if(
        rvars.begin(), rvars.end(), [&rvar_kept](const ReductionVariable &rv) {
  ...
  // Compute the predicates for the intermediate Func and the new update
  // definition
  ...
  for (size_t i = 0; i < update_vals.size(); i++) {
    ...
    val =
        substitute_self_reference(val, func_name, intm.function(), vars_rename);
    update_vals[i] = val;
  }
  ...
  vector<StorageDim> &intm_storage_dims =
      intm.function().schedule().storage_dims();
  internal_assert(intm_storage_dims.size() ==
                  function.schedule().storage_dims().size() +
                      vars_rename.size());
  ...
  for (size_t i = 0; i < f_values.size(); ++i) {
    if (!prover_result.ys[i].var.empty()) {
      Expr r = (values.size() == 1) ? Expr(intm(f_load_args))
                                    : Expr(intm(f_load_args)[i]);
      replacements.emplace(prover_result.ys[i].var, r);
    }

    if (!prover_result.xs[i].var.empty()) {
      Expr prev_val =
          Call::make(intm.output_types()[i], func_name, f_store_args,
                     Call::CallType::Halide, FunctionPtr(), i);
...
void Stage::split(const string &old, const string &outer, const string &inner,
  ...
  for (size_t i = 0; i < dims.size(); i++) {
    ...
    for (int j = 0; j < 2; j++) {
      if (var_name_match(dims[i].var, new_names[j]) && new_names[j] != old) {
        user_error << "In schedule for " << name() << ", can't create var "
                   << new_names[j] << " using a split or tile, because "
                   << new_names[j]
  ...
  if (!found) {
    user_error << "In schedule for " << name()
               << ", could not find split dimension: " << old << "\n"
               << dump_argument_list();
  }
  ...
  if (!definition.is_init()) {
    user_assert(tail != TailStrategy::ShiftInwards)
        ...
        << " ShiftInwards is not a legal tail strategy for update definitions, "
           "as"
  ...
  Split split = {old_name, outer_name, inner_name,     factor,
                 exact,    tail,       Split::SplitVar};
  definition.schedule().splits().push_back(split);
}

Stage &Stage::split(VarOrRVar old, VarOrRVar outer, VarOrRVar inner,
                    Expr factor, TailStrategy tail) {
  if (old.is_rvar) {
    user_assert(outer.is_rvar) << "Can't split RVar " << old.name()
                               << " into Var " << outer.name() << "\n";
    user_assert(inner.is_rvar) << "Can't split RVar " << old.name()
                               << " into Var " << inner.name() << "\n";
  } else {
    user_assert(!outer.is_rvar) << "Can't split Var " << old.name()
                                << " into RVar " << outer.name() << "\n";
    user_assert(!inner.is_rvar) << "Can't split Var " << old.name()
                                << " into RVar " << inner.name() << "\n";
  }
...
Stage &Stage::fuse(VarOrRVar inner, VarOrRVar outer, VarOrRVar fused) {
  ...
  if (!found_outer) {
    user_error << "In schedule for " << name()
               << ", could not find outer fuse dimension: " << outer.name()
               << "\n"
               << dump_argument_list();
  }
  ...
  for (size_t i = 0; (!found_inner) && i < dims.size(); i++) {
    if (var_name_match(dims[i].var, inner.name())) {
      ...
      if (dims[i].is_rvar()) {
        dims[i].dim_type = (dims[i].dim_type == Dim::Type::PureRVar) &&
  ...
  if (!found_inner) {
    user_error << "In schedule for " << name()
               << ", could not find inner fuse dimension: " << inner.name()
               << "\n"
               << dump_argument_list();
  }

  // Add the fuse to the splits list
  Split split = {fused_name,     outer_name, inner_name,
                 Expr(),         true,       TailStrategy::RoundUp,
                 Split::FuseVars};
  definition.schedule().splits().push_back(split);
  return *this;
}
...
Stage Stage::specialize(Expr condition) {
  user_assert(condition.type().is_bool())
      << "Argument passed to specialize must be of type bool\n";
  ...
  if (!check.offending_var.empty()) {
    user_error
        << "Specialization condition " << condition << " for " << name()
  ...
  user_assert(specializations.empty() ||
              specializations.back().failure_message.empty())
...
void Stage::specialize_fail(const std::string &message) {
  user_assert(!message.empty())
      << "Argument passed to specialize_fail() must not be empty.\n";
  const vector<Specialization> &specializations = definition.specializations();
  user_assert(specializations.empty() ||
              specializations.back().failure_message.empty())
...
Stage &Stage::purify(VarOrRVar old_var, VarOrRVar new_var) {
  user_assert(old_var.is_rvar && !new_var.is_rvar)
      << "In schedule for " << name() << ", can't rename "
      << (old_var.is_rvar ? "RVar " : "Var ") << old_var.name() << " to "
      << (new_var.is_rvar ? "RVar " : "Var ") << new_var.name()
      << "; purify must take a RVar as old_Var and a Var as new_var\n";

  debug(4) << "In schedule for " << name() << ", purify RVar " << old_var.name()
           << " to Var " << new_var.name() << "\n";
  ...
  if (!found) {
    user_error << "In schedule for " << name()
               << ", could not find rename dimension: " << old_var.name()
               << "\n"
               << dump_argument_list();
  }

  Split split = {
      old_name,         new_name, "", 1, false, TailStrategy::RoundUp,
      Split::PurifyRVar};
  definition.schedule().splits().push_back(split);
  return *this;
}
...
void Stage::remove(const string &var) {
  ...
  if (!found) {
    user_error << "In schedule for " << name()
               << ", could not find remove dimension: " << var << "\n"
               << dump_argument_list();
  }
  ...
  auto should_remove = [&removed_vars](const string &var) {
    const auto &iter =
        std::find_if(removed_vars.begin(), removed_vars.end(),
                     [&var](const string &rv) { return rv == var; });
  ...
  for (size_t i = splits.size(); i > 0; i--) {
    ...
    if (splits[i - 1].is_fuse()) {
      ...
      if (splits[i - 1].inner == old_name || splits[i - 1].outer == old_name) {
        user_error << "In schedule for " << name() << ", can't remove variable "
      ...
      debug(4) << "    replace/rename " << splits[i - 1].old_var << " into "
               << splits[i - 1].outer << "\n";
...
Stage &Stage::rename(VarOrRVar old_var, VarOrRVar new_var) {
  if (old_var.is_rvar) {
    user_assert(new_var.is_rvar)
        << "In schedule for " << name() << ", can't rename RVar "
        << old_var.name() << " to Var " << new_var.name() << "\n";
  } else {
    user_assert(!new_var.is_rvar)
        << "In schedule for " << name() << ", can't rename Var "
        << old_var.name() << " to RVar " << new_var.name() << "\n";
  }
...
Stage &Stage::tensor_core(const Parameter &paramA, const Parameter &paramB) {
  ...
  vector<Expr> predicates = definition.split_predicate();
  string old_name = rvars[0].var;
  string outer_name = rvars[0].var;
  string kernel = rvars[0].var;
  string inner_name = "";
  Expr factor = make_const(Int(32), 16);
  bool exact = true;
  TailStrategy tail = TailStrategy::GuardWithIf;
  Split split = {old_name, outer_name, inner_name,     factor,
                 exact,    tail,       Split::SplitVar};
  apply_outer_split_directive(split, rvars, predicates, args, values);
  vector<Dim> &dims = definition.schedule().dims();
  std::vector<string> vars_to_split;
  for (int i = 1; i < (int)dims.size() - 1; i++) {
    if (!dims[i].is_rvar())
      vars_to_split.push_back(dims[i].var);
  }

  std::vector<string> blocks;
  std::vector<string> threads;
  for (int i = 0; i < 1; i++) {
    old_name = vars_to_split[i];
    outer_name = vars_to_split[i];
    inner_name = old_name + "_i";
    if (i == 0)
      factor = 256;
    else
      factor = 4;
    split = {old_name, outer_name, inner_name,     factor,
             exact,    tail,       Split::SplitVar};

    definition.schedule().splits().push_back(split);
    threads.push_back(inner_name);
    blocks.push_back(outer_name);
    Dim d = {inner_name, ForType::Serial, DeviceAPI::None, Dim::Type::PureVar};
    dims.insert(dims.end() - 1, d);
  }
  Var kernelt(kernel);
  Var threadidx(threads[0]);

  Var blockidx(blocks[0]);

  reorder(kernelt, threadidx, blockidx);
  ...
  std::cout << "TN MAC " << function.definition().schedule().tensorOp().name
            << std::endl;
  std::cout << "A Mat " << paramA.name() << std::endl;
  std::cout << "A Mat " << paramB.name() << std::endl;
  definition.schedule().tensor_core() = true;
  return *this;
}
...
Stage &Stage::atomic(bool override_associativity_test) {
  ...
  definition.schedule().override_atomic_associativity_test() =
      override_associativity_test;
  return *this;
}
...
Stage &Stage::tile(VarOrRVar x, VarOrRVar y, VarOrRVar xo, VarOrRVar yo,
...
Stage &Stage::tile(VarOrRVar x, VarOrRVar y, VarOrRVar xi, VarOrRVar yi,
                   Expr xfactor, Expr yfactor, TailStrategy tail) {
    ...
    if (!dims[idx[i]].is_pure()) {
      for (size_t j = i + 1; !associativity_proven && j < idx.size(); j++) {
        if (!dims[idx[j]].is_pure() && (idx[i] > idx[j])) {
          // Generate an error if the operator is not both associative and
          // commutative.
          const auto &prover_result =
              prove_associativity(func_name, args, values);
...
Stage &Stage::gpu_threads(VarOrRVar tx, VarOrRVar ty, VarOrRVar tz,
...
Stage &Stage::gpu_blocks(VarOrRVar bx, VarOrRVar by, VarOrRVar bz,
                         DeviceAPI device_api) {
  set_dim_device_api(bx, device_api);
  set_dim_device_api(by, device_api);
...
Stage &Stage::gpu(VarOrRVar bx, VarOrRVar by, VarOrRVar tx, VarOrRVar ty,
                  DeviceAPI device_api) {
  return gpu_blocks(bx, by).gpu_threads(tx, ty);
}

Stage &Stage::gpu(VarOrRVar bx, VarOrRVar by, VarOrRVar bz, VarOrRVar tx,
                  VarOrRVar ty, VarOrRVar tz, DeviceAPI device_api) {
...
Stage &Stage::gpu_tile(VarOrRVar x, VarOrRVar y, VarOrRVar bx, VarOrRVar by,
                       VarOrRVar tx, VarOrRVar ty, Expr x_size, Expr y_size,
                       TailStrategy tail, DeviceAPI device_api) {
...
Stage &Stage::gpu_tile(VarOrRVar x, VarOrRVar y, VarOrRVar tx, VarOrRVar ty,
                       Expr x_size, Expr y_size, TailStrategy tail,
                       DeviceAPI device_api) {
  return gpu_tile(x, y, x, y, tx, ty, x_size, y_size, tail, device_api);
}

Stage &Stage::gpu_tile(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar bx,
                       VarOrRVar by, VarOrRVar bz, VarOrRVar tx, VarOrRVar ty,
                       VarOrRVar tz, Expr x_size, Expr y_size, Expr z_size,
                       TailStrategy tail, DeviceAPI device_api) {
  split(x, bx, tx, x_size, tail);
...
Stage &Stage::gpu_tile(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar tx,
                       VarOrRVar ty, VarOrRVar tz, Expr x_size, Expr y_size,
                       Expr z_size, TailStrategy tail, DeviceAPI device_api) {
  return gpu_tile(x, y, z, x, y, z, tx, ty, tz, x_size, y_size, z_size, tail,
                  device_api);
}
...
Stage &Stage::prefetch(const Func &f, VarOrRVar var, Expr offset,
  ...
  PrefetchDirective prefetch = {f.name(), var.name(), offset, strategy,
                                Parameter()};
  definition.schedule().prefetches().push_back(prefetch);
  return *this;
}

Stage &Stage::prefetch(const Internal::Parameter &param, VarOrRVar var,
                       Expr offset, PrefetchBoundStrategy strategy) {
  PrefetchDirective prefetch = {param.name(), var.name(), offset, strategy,
                                param};
  definition.schedule().prefetches().push_back(prefetch);
  return *this;
}
...
Stage &Stage::compute_with(LoopLevel loop_level,
                           const map<string, LoopAlignStrategy> &align) {
  loop_level.lock();
  ...
  Definition &original_def = (stage_index == 0)
                                 ? function.definition()
                                 : function.update(stage_index - 1);
  ...
  if (!fuse_level.level.lock().is_inlined()) {
    user_warning << name() << " already has a compute_with at "
                 << fuse_level.level.to_string()
                 << ". Replacing it with a new compute_with at "
                 << loop_level.to_string() << "\n";
  }
...
Stage &
Stage::compute_with(LoopLevel loop_level,
...
Stage &
Stage::compute_with(Stage s, VarOrRVar var,
                    const vector<pair<VarOrRVar, LoopAlignStrategy>> &align) {
...
void validate_wrapper(const string &name,
                      const map<string, FunctionPtr> &wrappers,
    ...
    for (const auto &it : wrappers) {
      ...
      const auto &fs_iter =
          std::find_if(fs.begin(), fs.end(),
                       [&it](const Func &f) { return f.name() == it.first; });
      bool in_fs = fs_iter != fs.end();

      if (in_fs) {
        user_assert(it.second.same_as(wrapper))
            << it.first << " should have shared the same wrapper as "
            << fs[0].name() << "\n";
      } else {
        user_assert(!it.second.same_as(wrapper))
            << "Redefinition of shared wrapper [" << name << " -> "
            << Function(wrapper).name() << "] in " << fs[0].name()
            << " is illegal since " << it.first
...
Func create_clone_wrapper(Function wrapped_fn, string wrapper_name) {
  ...
  wrapped_fn.deep_copy(wrapper.name(), wrapper.function().get_contents(),
                       empty);
  return wrapper;
}

Func get_wrapper(Function wrapped_fn, string wrapper_name,
                 const vector<Func> &fs, bool clone) {
  ...
  const auto &iter =
      fs.empty() ? wrappers.find("") : wrappers.find(fs[0].name());
...
Func Func::clone_in(const vector<Func> &fs) {
  if (fs.empty()) {
    user_error
        << "Could not create a clone wrapper for an empty list of Funcs\n";
  }
  invalidate_cache();
...
Func Func::copy_to_device(DeviceAPI d) {
  user_assert(defined()) << "copy_to_device on Func " << name()
                         << " with no definition\n";
  user_assert(outputs() == 1) << "copy_to_device on a Tuple-valued Func "
  ...
  user_assert(call) << "Func " << name()
                    << " is scheduled as copy_to_host/device, "
                    << "but has value: " << value() << "\n"
  ...
  func.define_extern("halide_buffer_copy", {buffer, device_interface},
                     {call->type},
                     func.args(), // Reuse the existing dimension names
                     NameMangling::C, d);
  return *this;
}

Func Func::copy_to_host() {
  user_assert(defined()) << "copy_to_host on Func " << name()
                         << " with no definition\n";
  user_assert(outputs() == 1) << "copy_to_host on a Tuple-valued Func "
                              << name() << " not yet supported\n";
  user_assert(!has_update_definition())
...
Func &Func::bound(Var var, Expr min, Expr extent) {
  user_assert(!min.defined() || Int(32).can_represent(min.type()))
      << "Can't represent min bound in int32\n";
  user_assert(extent.defined())
      << "Extent bound of a Func can't be undefined\n";
  user_assert(Int(32).can_represent(extent.type()))
      << "Can't represent extent bound in int32\n";
  ...
  user_assert(found) << "Can't bound variable " << var.name() << " of function "
                     << name() << " because " << var.name()
...
Func &Func::set_estimate(Var var, Expr min, Expr extent) {
  invalidate_cache();
  bool found = func.is_pure_arg(var.name());
  user_assert(found) << "Can't provide an estimate on variable " << var.name()
                     << " of function " << name() << " because " << var.name()
                     << " is not one of the pure variables of " << name()
                     << ".\n";
...
Func &Func::set_estimates(const std::vector<std::pair<Expr, Expr>> &estimates) {
  ...
  user_assert(estimates.size() == a.size())
      ...
      << "but the estimates passed to set_estimates contains "
      << estimates.size() << " pairs.\n";
...
Func &Func::align_bounds(Var var, Expr modulus, Expr remainder) {
  ...
  user_assert(Int(32).can_represent(modulus.type()))
      << "Can't represent modulus as int32\n";
  user_assert(Int(32).can_represent(remainder.type()))
      << "Can't represent remainder as int32\n";
  ...
  bool found = func.is_pure_arg(var.name());
  user_assert(found) << "Can't align bounds of variable " << var.name()
                     << " of function " << name() << " because " << var.name()
                     << " is not one of the pure variables of " << name()
                     << ".\n";
...
Func &Func::tile(VarOrRVar x, VarOrRVar y, VarOrRVar xo, VarOrRVar yo,
                 VarOrRVar xi, VarOrRVar yi, Expr xfactor, Expr yfactor,
                 TailStrategy tail) {
  invalidate_cache();
  Stage(func, func.definition(), 0)
      .tile(x, y, xo, yo, xi, yi, xfactor, yfactor, tail);
  return *this;
}

Func &Func::tile(VarOrRVar x, VarOrRVar y, VarOrRVar xi, VarOrRVar yi,
                 Expr xfactor, Expr yfactor, TailStrategy tail) {
  invalidate_cache();
...
Func &Func::gpu_threads(VarOrRVar tx, VarOrRVar ty, VarOrRVar tz,
                        DeviceAPI device_api) {
  invalidate_cache();
...
Func &Func::gpu_blocks(VarOrRVar bx, VarOrRVar by, VarOrRVar bz,
                       DeviceAPI device_api) {
  invalidate_cache();
...
Func &Func::gpu(VarOrRVar bx, VarOrRVar by, VarOrRVar tx, VarOrRVar ty,
                DeviceAPI device_api) {
  invalidate_cache();
...
Func &Func::gpu(VarOrRVar bx, VarOrRVar by, VarOrRVar bz, VarOrRVar tx,
                VarOrRVar ty, VarOrRVar tz, DeviceAPI device_api) {
  invalidate_cache();
...
Func &Func::gpu_tile(VarOrRVar x, VarOrRVar bx, VarOrRVar tx, Expr x_size,
                     TailStrategy tail, DeviceAPI device_api) {
  invalidate_cache();
  Stage(func, func.definition(), 0)
      .gpu_tile(x, bx, tx, x_size, tail, device_api);
  return *this;
}

Func &Func::gpu_tile(VarOrRVar x, VarOrRVar tx, Expr x_size, TailStrategy tail,
                     DeviceAPI device_api) {
  invalidate_cache();
...
Func &Func::gpu_tile(VarOrRVar x, VarOrRVar y, VarOrRVar bx, VarOrRVar by,
                     VarOrRVar tx, VarOrRVar ty, Expr x_size, Expr y_size,
                     TailStrategy tail, DeviceAPI device_api) {
  invalidate_cache();
  Stage(func, func.definition(), 0)
...
Func &Func::gpu_tile(VarOrRVar x, VarOrRVar y, VarOrRVar tx, VarOrRVar ty,
                     Expr x_size, Expr y_size, TailStrategy tail,
                     DeviceAPI device_api) {
  invalidate_cache();
  Stage(func, func.definition(), 0)
...
Func &Func::gpu_tile(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar bx,
                     VarOrRVar by, VarOrRVar bz, VarOrRVar tx, VarOrRVar ty,
                     VarOrRVar tz, Expr x_size, Expr y_size, Expr z_size,
                     TailStrategy tail, DeviceAPI device_api) {
  invalidate_cache();
  Stage(func, func.definition(), 0)
      .gpu_tile(x, y, z, bx, by, bz, tx, ty, tz, x_size, y_size, z_size, tail,
                device_api);
  return *this;
}

Func &Func::gpu_tile(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar tx,
                     VarOrRVar ty, VarOrRVar tz, Expr x_size, Expr y_size,
                     Expr z_size, TailStrategy tail, DeviceAPI device_api) {
  invalidate_cache();
  Stage(func, func.definition(), 0)
...
Func &Func::shader(Var x, Var y, Var c, DeviceAPI device_api) {
  ...
  for (size_t i = 0; i < sched.bounds().size(); i++) {
    if (c.name() == sched.bounds()[i].var) {
      constant_bounds =
          is_const(sched.bounds()[i].min) && is_const(sched.bounds()[i].extent);
      break;
    }
  }
  user_assert(constant_bounds) << "The color channel for image loops must have "
                                  "constant bounds, e.g., .bound(c, 0, 3).\n";
  return *this;
}
...
Func &Func::prefetch(const Internal::Parameter &param, VarOrRVar var,
                     Expr offset, PrefetchBoundStrategy strategy) {
  invalidate_cache();
...
Func &Func::reorder_storage(Var x, Var y) {
  ...
  user_error << "Could not find variables " << x.name() << " and " << y.name()
             << " to reorder in schedule.\n";
  return *this;
}
...
Func &
Func::compute_with(Stage s, VarOrRVar var,
                   const vector<pair<VarOrRVar, LoopAlignStrategy>> &align) {
  invalidate_cache();
...
Func &Func::compute_root() { return compute_at(LoopLevel::root()); }
...
Func &Func::store_at(Func f, RVar var) { return store_at(LoopLevel(f, var)); }

Func &Func::store_at(Func f, Var var) { return store_at(LoopLevel(f, var)); }

Func &Func::store_root() { return store_at(LoopLevel::root()); }

Func &Func::compute_inline() { return compute_at(LoopLevel::inlined()); }
...
Stage Func::update(int idx) {
  user_assert(idx < num_update_definitions())
      << "Call to update with index larger than last defined update stage for "
         "Func \""
      << name() << "\".\n";
  invalidate_cache();
...
FuncRef::FuncRef(Internal::Function f, const vector<Expr> &a,
                 int placeholder_pos, int count)
...
vector<Expr> FuncRef::args_with_implicit_vars(const vector<Expr> &e) const {
  ...
  for (size_t i = 0; i < a.size(); i++) {
    user_assert(a[i].defined()) << "Argument " << (i + 1) << " in call to \""
                                << func.name() << "\" is undefined.\n";
  }
  for (size_t i = 0; i < e.size(); i++) {
    user_assert(e[i].defined()) << "Value " << (i + 1) << " in definition of \""
                                << func.name() << "\" is undefined.\n";
  }
  ...
  if (count.count > 0) {
    if (func.has_pure_definition()) {
      ...
      user_assert(implicit_count >= count.count)
          << "The update definition of " << func.name() << " uses "
          << count.count
      ...
      internal_assert(implicit_count == 0)
          << "Pure definition can't possibly already have implicit variables "
             "defined\n";

      Internal::debug(2) << "Adding " << count.count
                         << " implicit vars to LHS of " << func.name() << "\n";
...
Stage FuncRef::operator=(Expr e) { return (*this) = Tuple(e); }
...
Stage FuncRef::operator=(const Tuple &e) {
  if (!func.has_pure_definition()) {
    ...
    // Find implicit args in the expr and add them to the args list before
    // calling define
...
Func define_base_case(Internal::Function func, const vector<Expr> &a,
                      const Tuple &e) {
  Func f(func);
...
Stage FuncRef::func_ref_update(const Tuple &e, int init_val) {
  ...
  vector<Expr> expanded_args = args_with_implicit_vars(e.as_vector());
  FuncRef self_ref =
      define_base_case(func, expanded_args, Tuple(init_values))(expanded_args);
...
Stage FuncRef::func_ref_update(Expr e, int init_val) {
  ...
  FuncRef self_ref = define_base_case(func, expanded_args,
                                      cast(e.type(), init_val))(expanded_args);
...
size_t FuncRef::size() const { return func.outputs(); }

FuncTupleElementRef::FuncTupleElementRef(const FuncRef &ref,
                                         const std::vector<Expr> &args, int idx)
...
Realization Func::realize(int x_size, int y_size, int z_size,
                          const Target &target, const ParamMap &param_map) {
...
OutputImageParam Func::output_buffer() const {
  user_assert(defined()) << "Can't access output buffer of undefined Func.\n";
...
vector<OutputImageParam> Func::output_buffers() const {
  user_assert(defined()) << "Can't access output buffers of undefined Func.\n";
...
std::string Func::source_location() const {
  user_assert(defined())
      << "A Func with no definition has no source_location\n";
  return func.definition().source_location();
}

Module Func::compile_to_module(const vector<Argument> &args,
...
void Func::compile_to_bitcode(const string &filename,
...
void Func::compile_to_llvm_assembly(const string &filename,
                                    const vector<Argument> &args,
...
void Func::compile_to_object(const string &filename,
                             const vector<Argument> &args,
                             const string &fn_name, const Target &target) {
...
void Func::compile_to_header(const string &filename,
                             const vector<Argument> &args,
                             const string &fn_name, const Target &target) {
...
void Func::compile_to_lowered_stmt(const string &filename,
                                   const vector<Argument> &args,
                                   StmtOutputFormat fmt, const Target &target) {
  pipeline().compile_to_lowered_stmt(filename, args, fmt, target);
}

void Func::print_loop_nest() { pipeline().print_loop_nest(); }
...
void Func::compile_to_multitarget_static_library(
    const std::string &filename_prefix, const std::vector<Argument> &args,
    const std::vector<Target> &targets) {
  pipeline().compile_to_multitarget_static_library(filename_prefix, args,
                                                   targets);
}

void Func::compile_to_assembly(const string &filename,
                               const vector<Argument> &args,
                               const string &fn_name, const Target &target) {
...
void Func::set_custom_do_par_for(int (*cust_do_par_for)(
    void *, int (*)(void *, int, uint8_t *), int, int, uint8_t *)) {
  pipeline().set_custom_do_par_for(cust_do_par_for);
}

void Func::set_custom_do_task(int (*cust_do_task)(
    void *, int (*)(void *, int, uint8_t *), int, uint8_t *)) {
  pipeline().set_custom_do_task(cust_do_task);
}

void Func::set_custom_trace(int (*trace_fn)(void *,
                                            const halide_trace_event_t *)) {
...
void Func::add_custom_lowering_pass(IRMutator *pass,
...
void Func::compile_jit(const Target &target) { pipeline().compile_jit(target); }
