...
Expr IRMutator::visit(const Load *op) {
  ...
  return Load::make(op->type, op->name, std::move(index), op->image, op->param,
                    std::move(predicate), op->alignment);
}
...
Stmt IRMutator::visit(const For *op) {
  ...
  return For::make(op->name, std::move(min), std::move(extent), op->for_type,
                   op->device_api, std::move(body));
}

Stmt IRMutator::visit(const DoWhile *op) {
  Expr min = mutate(op->min);
  Expr cond = mutate(op->cond);
  Expr step = mutate(op->step);
  Stmt body = mutate(op->body);
  if (min.same_as(op->min) && cond.same_as(op->cond) &&
      step.same_as(op->step) && body.same_as(op->body)) {
    return op;
  }
  return DoWhile::make(op->name, std::move(min), std::move(cond),
                       std::move(step), op->for_type, op->device_api,
                       std::move(body));
}
...
Stmt IRMutator::visit(const Store *op) {
  ...
  return Store::make(op->name, std::move(value), std::move(index), op->param,
                     std::move(predicate), op->alignment);
}
...
Stmt IRMutator::visit(const Allocate *op) {
  ...
  return Allocate::make(op->name, op->type, op->memory_type, new_extents,
                        std::move(condition), std::move(body),
                        std::move(new_expr), op->free_function);
}

Stmt IRMutator::visit(const Free *op) { return op; }
...
Stmt IRMutator::visit(const TensorOp *op) {
  Stmt body = mutate(op->body);
  // Expr condition = mutate(op->condition);
  ...
  if (!args_changed && body.same_as(op->body)) {
    return op;
  }
  return TensorOp::make(op->name, op->types, new_args, std::move(body));
}
...
Stmt IRMutator::visit(const Acquire *op) {
  ...
  Stmt body = mutate(op->body);
  if (sema.same_as(op->semaphore) && body.same_as(op->body) &&
...
Stmt IRMutator::visit(const Atomic *op) {
  ...
  if (body.same_as(op->body)) {
    ...
    return Atomic::make(op->producer_name, op->mutex_name, std::move(body));
  }
}
