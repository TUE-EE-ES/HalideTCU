...
Module lower(const vector<Function> &output_funcs, const string &pipeline_name,
             const Target &t, const vector<Argument> &args,
             const LinkageType linkage_type, const vector<Stmt> &requirements,
             bool trace_pipeline, const vector<IRMutator *> &custom_passes) {
  std::vector<std::string> namespaces;
  std::string simple_pipeline_name =
      extract_namespaces(pipeline_name, namespaces);

  Module result_module(simple_pipeline_name, t);

  // Compute an environment
  map<string, Function> env;
  map<string, Function> env_og;
  for (Function f : output_funcs) {
    populate_environment(f, env);
    populate_environment(f, env_og);
  }
  ...
  if (t.has_gpu_feature() || t.has_feature(Target::OpenGLCompute) ||
      t.has_feature(Target::OpenGL) || t.has_feature(Target::HexagonDma) ||
      (t.arch != Target::Hexagon &&
       (t.features_any_of({Target::HVX_64, Target::HVX_128})))) {
    ...
    debug(2) << "Lowering after selecting a GPU API for extern stages:\n"
             << s << "\n\n";
    debug(1) << "Injecting tensor ops...\n";
    s = inject_tensor_ops(s, env_og);
    debug(2) << "Lowering after injecting tensor ops:\n" << s << "\n\n";
  }
  ...
  s = simplify(s);
  debug(1) << "Lowering before  LICM simplification:\n" << s << "\n\n";
  s = loop_invariant_code_motion(s);
  debug(1) << "Lowering after LICM simplification:\n" << s << "\n\n";
  ...
  for (const auto &out : outputs) {
    for (Parameter buf : out.output_buffers()) {
      public_args.push_back(Argument(buf.name(), Argument::OutputBuffer,
                                     buf.type(), buf.dimensions(),
                                     buf.get_argument_estimates()));
    }
  }
  ...
  class StrengthenRefs : public IRMutator {
    ...
    Expr visit(const Call *c) override {
      ...
      if (c->func.defined()) {
        ...
        ptr.strengthen();
        expr = Call::make(c->type, c->name, c->args, c->call_type, ptr,
                          c->value_index, c->image, c->param);
      }
      return expr;
    }
  };
...
Stmt lower_main_stmt(const std::vector<Function> &output_funcs,
                     ...
                     const std::vector<Stmt> &requirements, bool trace_pipeline,
  ...
  vector<InferredArgument> inferred_args =
      infer_arguments(Stmt(), output_funcs);
  ...
  Module module =
      lower(output_funcs, pipeline_name, t, args, LinkageType::External,
            requirements, trace_pipeline, custom_passes);
