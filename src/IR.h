...
struct Provide : public StmtNode<Provide> {
  ...
  static Stmt make(const std::string &name, const std::vector<Expr> &values,
...
struct Allocate : public StmtNode<Allocate> {
   ...
   * size. */
  static int32_t constant_allocation_size(const std::vector<Expr> &extents,
...
struct Call : public ExprNode<Call> {
  ...
  typedef enum {
    ...
    Extern,          ///< A call to an external C-ABI function, possibly with
                     ///< side-effects
    ExternCPlusPlus, ///< A call to an external C-ABI function, possibly with
                     ///< side-effects
    PureExtern,   ///< A call to a guaranteed-side-effect-free external function
    Halide,       ///< A call to a Func
    Intrinsic,    ///< A possibly-side-effecty compiler intrinsic, which has
  ...
  // intrinsics have an enum entry here, but as a matter of style, it is
  // recommended.) Note that these are only used in the API; inside the node,
  // they are translated into a name. (To recover the name, call
  // get_intrinsic_name().)
  //
  ...
  enum IntrinsicOp {
    ...
    mod_round_to_zero,
    mulhi_shr, // Compute high_half(arg[0] * arg[1]) >> arg[3]. Note that this
               // is a shift in addition to taking the upper half of multiply
               // result. arg[3] must be an unsigned integer immediate.
    popcount,
    prefetch,
    quiet_div,
    quiet_mod,
  ...
  HALIDE_EXPORT static ConstString buffer_get_dimensions, buffer_get_min,
      buffer_get_extent, buffer_get_stride, buffer_get_max, buffer_get_host,
      buffer_get_device, buffer_get_device_interface, buffer_get_shape,
      buffer_get_host_dirty, buffer_get_device_dirty, buffer_get_type,
      buffer_set_host_dirty, buffer_set_device_dirty, buffer_is_bounds_query,
      buffer_init, buffer_init_from_buffer, buffer_crop, buffer_set_bounds,
      trace;
  ...
  static Expr make(Type type, IntrinsicOp op, const std::vector<Expr> &args,
                   CallType call_type, FunctionPtr func = FunctionPtr(),
  ...
  static Expr make(Buffer<> image, const std::vector<Expr> &args) {
    return make(image.type(), image.name(), args, Image, FunctionPtr(), 0,
                image, Parameter());
  }

  /** Convenience constructor for loads from images parameters */
  static Expr make(Parameter param, const std::vector<Expr> &args) {
    return make(param.type(), param.name(), args, Image, FunctionPtr(), 0,
                Buffer<>(), param);
  }
  ...
  bool is_pure() const {
    return (call_type == PureExtern || call_type == Image ||
  ...
  bool is_extern() const {
    return (call_type == Extern || call_type == ExternCPlusPlus ||
...
struct For : public StmtNode<For> {
  ...
  static Stmt make(const std::string &name, Expr min, Expr extent,
                   ForType for_type, DeviceAPI device_api, Stmt body);

  bool is_unordered_parallel() const {
    return Halide::Internal::is_unordered_parallel(for_type);
  }
  bool is_parallel() const { return Halide::Internal::is_parallel(for_type); }

  static const IRNodeType _node_type = IRNodeType::For;
};

struct DoWhile : public StmtNode<DoWhile> {
  std::string name;
  Expr min, cond, step;
  ForType for_type;
  DeviceAPI device_api;
  ...
  static Stmt make(const std::string &name, Expr min, Expr cond, Expr step,
                   ForType for_type, DeviceAPI device_api, Stmt body);
  ...
  static const IRNodeType _node_type = IRNodeType::DoWhile;
};
...
// represents that a function was flagged as tensor MMA sequence of operations
// should have the name of the function, the args (indices) and the params to
// read/write;
// TODO add the types and extents
// also a body  which is the actual matmul (mma/name = , arg0 , arg1 ,arg2 ,
// arg3 , arg4 , arg5 etc);

struct TensorOp : public StmtNode<TensorOp> {
  std::string name;
  std::vector<Type> types;
  Region bounds;
  // TensorOpDirective tensorOp;
  // Expr condition;
  std::vector<Expr> args;
  Stmt body;
  // Stmt kernel;
  ...
  static const IRNodeType _node_type = IRNodeType::TensorOp;
};
