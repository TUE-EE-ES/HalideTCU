...
Expr make_indeterminate_expression(Type type) {
  ...
  return Call::make(type, Call::indeterminate_expression, {counter++},
                    Call::Intrinsic);
}

Expr make_signed_integer_overflow(Type type) {
  static std::atomic<int> counter;
  return Call::make(type, Call::signed_integer_overflow, {counter++},
                    Call::Intrinsic);
}
...
void match_types(Expr &a, Expr &b) {
  ...
  // If type broadcasting has made the types match no additional casts are
  // needed
  if (ta == tb)
    return;
...
void match_types_bitwise(Expr &x, Expr &y, const char *op_name) {
  ...
  user_assert(x.type().is_int() || x.type().is_uint())
      << "The first argument to " << op_name
      << " must be an integer or unsigned integer";
  user_assert(y.type().is_int() || y.type().is_uint())
      << "The second argument to " << op_name
      << " must be an integer or unsigned integer";
  ...
  if (x.type().is_scalar() && y.type().is_vector()) {
    ...
    internal_assert(x.type().lanes() == y.type().lanes())
        << "Can't match types of differing widths";
  }
...
Expr halide_erf(Expr x_full) {
  user_assert(x_full.type() == Float(32))
      << "halide_erf only works for Float(32)";
...
Expr BufferBuilder::build() const {
  ...
  if (buffer_memory.defined()) {
    ...
    args[0] = Call::make(type_of<struct halide_buffer_t *>(), Call::alloca,
                         {sz}, Call::Intrinsic);
  }

  std::string shape_var_name = unique_name('t');
  Expr shape_var =
      Variable::make(type_of<halide_dimension_t *>(), shape_var_name);
  ...
  Expr shape_arg = Call::make(type_of<halide_dimension_t *>(),
                              Call::make_struct, shape, Call::Intrinsic);
  if (shape_memory.defined()) {
    args[8] = shape_arg;
  } else if (dimensions == 0) {
  ...
  if (host_dirty.defined()) {
    flags =
        select(host_dirty, make_const(UInt(64), halide_buffer_flag_host_dirty),
               make_zero(UInt(64)));
  }
  if (device_dirty.defined()) {
    flags =
        flags | select(device_dirty,
                       make_const(UInt(64), halide_buffer_flag_device_dirty),
                       make_zero(UInt(64)));
  }
  args[9] = flags;

  Expr e = Call::make(type_of<struct halide_buffer_t *>(), Call::buffer_init,
                      args, Call::Extern);
...
Expr remove_likelies(Expr e) { return RemoveLikelies().mutate(e); }

Stmt remove_likelies(Stmt s) { return RemoveLikelies().mutate(s); }

Expr requirement_failed_error(Expr condition, const std::vector<Expr> &args) {
  return Internal::Call::make(Int(32), "halide_error_requirement_failed",
                              {stringify({condition}), combine_strings(args)},
                              Internal::Call::Extern);
}

Expr memoize_tag_helper(Expr result,
  ...
  return Internal::Call::make(t, Internal::Call::memoize_expr, args,
...
// A vectorizable sine and cosine implementation. Based on syrah fast vector
// math
...
Expr fast_sin_cos(Expr x_full, bool is_sin) {
  ...
  Expr k_mod4 = k % 4;
  Expr sin_usecos = is_sin ? ((k_mod4 == 1) || (k_mod4 == 3))
                           : ((k_mod4 == 0) || (k_mod4 == 2));
  ...
  Expr x2 = x * x;
  Expr tri_func =
      outside * (x2 * (x2 * (x2 * (x2 * (x2 * c10 + c8) + c6) + c4) + c2) + 1);
  return select(flip_sign, -tri_func, tri_func);
}

} // namespace

Expr fast_sin(Expr x_full) { return fast_sin_cos(x_full, true); }

Expr fast_cos(Expr x_full) { return fast_sin_cos(x_full, false); }
...
Expr print(const std::vector<Expr> &args) {
  ...
  Expr print_call = Internal::Call::make(
      Int(32), "halide_print", {combined_string}, Internal::Call::Extern);
...
Expr require(Expr condition, const std::vector<Expr> &args) {
  ...
  return Internal::Call::make(
      args[0].type(), Internal::Call::require,
...
Expr saturating_cast(Type t, Expr e) {
  ...
  if (t.is_float() && e.type().is_float()) {
    ...
    if (e.type().is_float() && !t.is_float() && t.bits() >= e.type().bits()) {
      e = max(std::move(e),
              t.min()); // min values turn out to be always representable
...
Tuple tuple_select(const Tuple &condition, const Tuple &true_value,
                   const Tuple &false_value) {
  user_assert(condition.size() == true_value.size() &&
              true_value.size() == false_value.size())
...
Tuple tuple_select(const Expr &condition, const Tuple &true_value,
                   const Tuple &false_value) {
...
Expr unsafe_promise_clamped(Expr value, Expr min, Expr max) {
  user_assert(value.defined())
      << "unsafe_promise_clamped with undefined value.\n";
  Expr n_min_val =
      min.defined() ? lossless_cast(value.type(), min) : value.type().min();
  Expr n_max_val =
      max.defined() ? lossless_cast(value.type(), max) : value.type().max();

  // Min and max are allowed to be undefined with the meaning of no bound on
  // that side.

  return Internal::Call::make(
      value.type(), Internal::Call::unsafe_promise_clamped,
      {value, n_min_val, n_max_val}, Internal::Call::Intrinsic);
}
...
Expr operator&&(bool a, const Expr &b) { return std::move(b) && a; }
...
Expr operator||(bool a, const Expr &b) { return b || a; }

Expr operator!(Expr a) { return Internal::Not::make(std::move(a)); }
...
Expr cast(Type t, Expr a) {
  ...
  if (t.is_vector()) {
    if (a.type().is_scalar()) {
      return Internal::Broadcast::make(cast(t.element_of(), std::move(a)),
                                       t.lanes());
...
Expr clamp(Expr a, Expr min_val, Expr max_val) {
  ...
  user_assert(n_min_val.defined())
      << "Type mismatch in call to clamp. First argument (" << a
      << ") has type " << a.type() << ", but second argument (" << min_val
      << ") has type " << min_val.type() << ". Use an explicit cast.\n";
  Expr n_max_val = lossless_cast(a.type(), max_val);
  user_assert(n_max_val.defined())
      << "Type mismatch in call to clamp. First argument (" << a
      << ") has type " << a.type() << ", but third argument (" << max_val
      << ") has type " << max_val.type() << ". Use an explicit cast.\n";
  return Internal::Max::make(
      Internal::Min::make(std::move(a), std::move(n_max_val)),
      std::move(n_min_val));
}
...
Expr absd(Expr a, Expr b) {
  ...
  return Internal::Call::make(
      t.with_code(t.is_int() ? Type::UInt : t.code()), Internal::Call::absd,
...
Expr sin(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "sin_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "sin_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "sin_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr asin(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "asin_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "asin_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "asin_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr cos(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "cos_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "cos_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "cos_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr acos(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "acos_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "acos_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "acos_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr tan(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "tan_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "tan_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "tan_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr atan(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "atan_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "atan_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "atan_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr atan2(Expr y, Expr x) {
  ...
  if (y.type() == Float(64)) {
    x = cast<double>(x);
    return Internal::Call::make(Float(64), "atan2_f64",
                                {std::move(y), std::move(x)},
                                Internal::Call::PureExtern);
  } else if (y.type() == Float(16)) {
    x = cast<float16_t>(x);
    return Internal::Call::make(Float(16), "atan2_f16",
                                {std::move(y), std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    y = cast<float>(y);
    x = cast<float>(x);
    return Internal::Call::make(Float(32), "atan2_f32",
                                {std::move(y), std::move(x)},
                                Internal::Call::PureExtern);
  }
}
...
Expr sinh(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "sinh_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "sinh_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "sinh_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr asinh(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "asinh_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "asinh_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "asinh_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr cosh(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "cosh_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "cosh_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "cosh_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr acosh(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "acosh_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "acosh_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "acosh_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr tanh(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "tanh_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "tanh_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "tanh_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr atanh(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "atanh_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "atanh_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "atanh_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr sqrt(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "sqrt_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "sqrt_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "sqrt_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}

Expr hypot(Expr x, Expr y) { return sqrt(x * x + y * y); }

Expr exp(Expr x) {
  user_assert(x.defined()) << "exp of undefined Expr\n";
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "exp_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "exp_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "exp_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr log(Expr x) {
  ...
  if (x.type() == Float(64)) {
    return Internal::Call::make(Float(64), "log_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    return Internal::Call::make(Float(16), "log_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    return Internal::Call::make(Float(32), "log_f32",
                                {cast<float>(std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr pow(Expr x, Expr y) {
  ...
  if (x.type() == Float(64)) {
    ...
    return Internal::Call::make(Float(64), "pow_f64",
                                {std::move(x), std::move(y)},
                                Internal::Call::PureExtern);
  } else if (x.type() == Float(16)) {
    y = cast<float16_t>(std::move(y));
    return Internal::Call::make(Float(16), "pow_f16",
                                {std::move(x), std::move(y)},
                                Internal::Call::PureExtern);
  } else {
    x = cast<float>(std::move(x));
    y = cast<float>(std::move(y));
    return Internal::Call::make(Float(32), "pow_f32",
                                {std::move(x), std::move(y)},
                                Internal::Call::PureExtern);
  }
}
...
Expr fast_inverse(Expr x) {
  ...
  Type t = x.type();
  return Internal::Call::make(t, "fast_inverse_f32", {std::move(x)},
                              Internal::Call::PureExtern);
}
...
Expr fast_inverse_sqrt(Expr x) {
  ...
  Type t = x.type();
  return Internal::Call::make(t, "fast_inverse_sqrt_f32", {std::move(x)},
                              Internal::Call::PureExtern);
}
...
Expr floor(Expr x) {
  ...
  if (t.element_of() == Float(64)) {
    return Internal::Call::make(t, "floor_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (t.element_of() == Float(16)) {
    return Internal::Call::make(t, "floor_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    t = Float(32, t.lanes());
    if (t.is_int() || t.is_uint()) {
      ...
      return Internal::Call::make(t, "floor_f32", {cast(t, std::move(x))},
                                  Internal::Call::PureExtern);
    }
  }
}
...
Expr ceil(Expr x) {
  ...
  Type t = x.type();
  if (t.element_of() == Float(64)) {
    return Internal::Call::make(t, "ceil_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type().element_of() == Float(16)) {
    return Internal::Call::make(t, "ceil_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    t = Float(32, t.lanes());
    if (t.is_int() || t.is_uint()) {
      ...
      return Internal::Call::make(t, "ceil_f32", {cast(t, std::move(x))},
                                  Internal::Call::PureExtern);
    }
  }
}
...
Expr round(Expr x) {
  ...
  Type t = x.type();
  if (t.element_of() == Float(64)) {
    return Internal::Call::make(t, "round_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (t.element_of() == Float(16)) {
    return Internal::Call::make(t, "round_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    t = Float(32, t.lanes());
    if (t.is_int() || t.is_uint()) {
      ...
      return Internal::Call::make(t, "round_f32", {cast(t, std::move(x))},
                                  Internal::Call::PureExtern);
    }
  }
}
...
Expr trunc(Expr x) {
  ...
  Type t = x.type();
  if (t.element_of() == Float(64)) {
    return Internal::Call::make(t, "trunc_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (t.element_of() == Float(16)) {
    return Internal::Call::make(t, "trunc_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    t = Float(32, t.lanes());
    if (t.is_int() || t.is_uint()) {
      ...
      return Internal::Call::make(t, "trunc_f32", {cast(t, std::move(x))},
                                  Internal::Call::PureExtern);
    }
  }
}
...
Expr is_nan(Expr x) {
  ...
  if (x.type().element_of() == Float(64)) {
    return Internal::Call::make(t, "is_nan_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type().element_of() == Float(16)) {
    return Internal::Call::make(t, "is_nan_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    Type ft = Float(32, t.lanes());
    return Internal::Call::make(t, "is_nan_f32", {cast(ft, std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr is_inf(Expr x) {
  ...
  Type t = Bool(x.type().lanes());
  if (x.type().element_of() == Float(64)) {
    return Internal::Call::make(t, "is_inf_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type().element_of() == Float(16)) {
    return Internal::Call::make(t, "is_inf_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    Type ft = Float(32, t.lanes());
    return Internal::Call::make(t, "is_inf_f32", {cast(ft, std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr is_finite(Expr x) {
  ...
  Type t = Bool(x.type().lanes());
  if (x.type().element_of() == Float(64)) {
    return Internal::Call::make(t, "is_finite_f64", {std::move(x)},
                                Internal::Call::PureExtern);
  } else if (x.type().element_of() == Float(16)) {
    return Internal::Call::make(t, "is_finite_f16", {std::move(x)},
                                Internal::Call::PureExtern);
  } else {
    Type ft = Float(32, t.lanes());
    return Internal::Call::make(t, "is_finite_f32", {cast(ft, std::move(x))},
                                Internal::Call::PureExtern);
  }
}
...
Expr reinterpret(Type t, Expr e) {
  ...
  return Internal::Call::make(t, Internal::Call::reinterpret, {std::move(e)},
                              Internal::Call::PureIntrinsic);
}
...
Expr operator&(Expr x, Expr y) {
  ...
  Type t = x.type();
  return Internal::Call::make(t, Internal::Call::bitwise_and,
                              {std::move(x), std::move(y)},
                              Internal::Call::PureIntrinsic);
}
...
Expr operator&(Expr x, int y) {
  ...
  return Internal::Call::make(t, Internal::Call::bitwise_and,
                              {std::move(x), Internal::make_const(t, y)},
                              Internal::Call::PureIntrinsic);
}
...
Expr operator&(int x, Expr y) {
  ...
  return Internal::Call::make(t, Internal::Call::bitwise_and,
                              {Internal::make_const(t, x), std::move(y)},
                              Internal::Call::PureIntrinsic);
}
...
Expr operator|(Expr x, Expr y) {
  ...
  Type t = x.type();
  return Internal::Call::make(t, Internal::Call::bitwise_or,
                              {std::move(x), std::move(y)},
                              Internal::Call::PureIntrinsic);
}
...
Expr operator^(Expr x, Expr y) {
  ...
  Type t = x.type();
  return Internal::Call::make(t, Internal::Call::bitwise_xor,
                              {std::move(x), std::move(y)},
                              Internal::Call::PureIntrinsic);
}
...
Expr operator~(Expr x) {
  ...
  Type t = x.type();
  return Internal::Call::make(t, Internal::Call::bitwise_not, {std::move(x)},
                              Internal::Call::PureIntrinsic);
}
...
Expr operator<<(Expr x, Expr y) {
  ...
  match_bits(x, y);
  Type t = x.type();
  return Internal::Call::make(t, Internal::Call::shift_left,
                              {std::move(x), std::move(y)},
                              Internal::Call::PureIntrinsic);
}
...
Expr operator>>(Expr x, Expr y) {
  ...
  match_bits(x, y);
  Type t = x.type();
  return Internal::Call::make(t, Internal::Call::shift_right,
                              {std::move(x), std::move(y)},
                              Internal::Call::PureIntrinsic);
}
...
Expr popcount(Expr x) {
  ...
  return Internal::Call::make(t, Internal::Call::popcount, {std::move(x)},
                              Internal::Call::PureIntrinsic);
}
...
Expr likely(Expr e) {
  Type t = e.type();
  return Internal::Call::make(t, Internal::Call::likely, {std::move(e)},
                              Internal::Call::PureIntrinsic);
}
...
Expr strict_float(Expr e) {
  Type t = e.type();
  return Internal::Call::make(t, Internal::Call::strict_float, {std::move(e)},
                              Internal::Call::PureIntrinsic);
}
