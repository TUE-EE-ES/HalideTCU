...
string get_expr_str2(Expr expr) {
...
void Closure::found_buffer_ref(const string &name, Type type, bool read,
                               bool written, Halide::Buffer<> image) {
  if (!ignore.contains(name)) {
    Buffer &ref = buffers[name];
    ref.type =
        type.element_of(); // TODO: Validate type is the same as existing refs?
...
void Closure::visit(const TensorOp *op) { op->body.accept(this); }
...
void Closure::visit(const Atomic *op) {
  if (op->mutex_name != "") {
    found_buffer_ref(op->mutex_name, type_of<void *>(), true, true,
                     Halide::Buffer<>());
  }
  op->body.accept(this);
}
